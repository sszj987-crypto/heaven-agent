...
class MemoryPersistModule(PipelineModule):
    ...
    async def process(self, ctx: PipelineContext) -> PipelineContext:
        ...
        log.info("记忆落盘开始, date=%s, file=%s", today, file_path)

        # 构建当日记录
        content = self._format_entry(ctx)
        log.debug("记忆内容, soul=%s, user_msg_len=%d, reply_len=%d",
                  ctx.soul_profile.name if ctx.soul_profile else "未知",
                  len(ctx.user_message), len(ctx.response))
        ...
        log.info("记忆落盘完成, file=%s, size=%d bytes", file_path, file_size)
        return ctx
